...
const checks = [
  ['presença é separada do status bruto do horário', migrate.includes('confirmation_status') && frontend.includes('appointmentPresenceMeta')],
  ...
  ['dashboard recebe resumo e exceções de confirmação', server.includes('confirmationSummary') && server.includes('confirmationAttention') && frontend.includes('Confirmações de hoje')],
  ['dashboard não lista agendamentos cancelados como próximos', server.includes("AND a.status != 'CANCELLED'")],
  ['dashboard não usa PENDING como ação de confirmar presença', !frontend.includes("handleStatusChange(appt.id, 'CONFIRMED')")],
  ['notificações não chamam status bruto PENDING de pendência da dona', !frontend.includes('Agendamento pendente')],
  ['novo agendamento administrativo usa presença para cor imediata', frontend.includes('const presence = appointmentPresenceMeta(a.status, a.confirmationStatus)')],
  ['agenda exibe presença separada e confirmação manual', frontend.includes('appointment-presence-card') && frontend.includes('Confirmar manualmente')],
  ['portal da cliente usa status de presença', frontend.includes('appointmentPresenceMeta(a.status, a.confirmationStatus)')],
  ['sucesso do booking não afirma presença confirmada', frontend.includes('Agendamento realizado') && !frontend.includes('<span>Reserva confirmada</span>')],
  ['WhatsApp possui configuração de confirmação automática', whatsfrontend.includes('Confirmação de presença') && whatsfrontend.includes('confirmationOffsetHours') && whatsfrontend.includes('noResponseCutoffHours')],
  ['settings realmente carregam os campos novos', server.includes('confirmations_enabled as confirmationsEnabled') && whatsfrontend.includes('setSettings({ ...defaultSettings, ...res.data.settings })')],
  ...
  ['Agenda possui legenda de presença', frontend.includes('appointment-presence-legend') && css.includes('.appointment-presence-legend')],
  ...
  ['mensagem pós-booking explica confirmação por WhatsApp', frontend.includes('Mais perto do atendimento, você recebe a confirmação pelo WhatsApp')],
]
